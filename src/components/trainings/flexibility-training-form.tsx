...
interface FlexibilityTrainingFormProps {
  onBack: () => void;
}

export function FlexibilityTrainingForm({ onBack }: FlexibilityTrainingFormProps) {
  ...
  const onSubmit = async (data: FlexibilityTrainingFormData) => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Tu entrenamiento de flexibilidad ha sido registrado exitosamente.',
      });

      router.push('/trainings');
    } catch (error) {
      console.error('Error creating flexibility training:', error);
      toast({
        title: 'Error',
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-4">
          ...
          <CardTitle>Nuevo Entrenamiento de Flexibilidad</CardTitle>
        </div>
      </CardHeader>
      <CardContent>
