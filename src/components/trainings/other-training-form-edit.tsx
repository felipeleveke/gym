...
interface OtherTrainingFormEditProps {
  training: any;
}

export function OtherTrainingFormEdit({ training }: OtherTrainingFormEditProps) {
  ...
  const onSubmit = async (data: OtherTrainingFormData) => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Tu entrenamiento ha sido actualizado exitosamente.',
      });

      router.push('/trainings');
    } catch (error) {
      console.error('Error updating other training:', error);
      toast({
        title: 'Error',
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-4">
          ...
          <CardTitle>Editar Entrenamiento</CardTitle>
        </div>
      </CardHeader>
      <CardContent>
